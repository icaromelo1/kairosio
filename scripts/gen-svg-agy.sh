...
cat > "$SCHEMA" <<JSON
{
  ...
  "properties": {
    "viewBox":    { "type": "string", "const": "0 0 ${VBW} ${VBH}" },
...
REGRAS OBRIGATORIAS:
1. viewBox exatamente \"0 0 ${VBW} ${VBH}\". Retorne no campo svg APENAS o conteudo interno, sem a tag <svg> externa, sem <?xml?>, sem comentarios <!-- -->.
...
open('$OUT_DIR/$KIND.svg','w').write('<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 $VBW $VBH\">\n' + svg + '\n</svg>\n')
