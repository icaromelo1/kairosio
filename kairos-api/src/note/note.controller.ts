...
export class NoteController {
  ...
  // notas são escopadas por servidor — sem servidor (convidado num mundo template) a
  ...
  findAll(@Request() req: any, @Query('mapId') mapId: string, @Query('objectId') objectId: string) {
    ...
    return this.notes.list(req.user.serverId, mapId, objectId)
  }

  @Post()
  create(@Request() req: any, @Body() dto: CreateNoteDto) {
    if (!req.user.serverId) {
      throw new ForbiddenException('Entre num servidor para usar as notas da estante.')
    }
    return this.notes.create(req.user.serverId, req.user.sub, dto)
  }

  @Patch(':id')
  update(@Request() req: any, @Param('id') id: string, @Body() dto: UpdateNoteDto) {
    return this.notes.update(req.user.serverId, id, dto)
  }

  @Delete(':id')
  remove(@Request() req: any, @Param('id') id: string) {
    return this.notes.remove(req.user.serverId, id)
  }
}
