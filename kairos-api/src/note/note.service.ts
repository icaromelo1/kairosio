...
export class NoteService {
  ...
  list(serverId: string | null, mapId: string, objectId: string): Promise<Note[]> {
  ...
  create(serverId: string | null, userId: string, dto: CreateNoteDto): Promise<Note> {
  ...
  async update(serverId: string | null, id: string, dto: UpdateNoteDto): Promise<Note> {
    const note = await this.findOne(id)
    if (note.serverId !== serverId) {
  ...
  async remove(serverId: string | null, id: string): Promise<{ deleted: string }> {
    const note = await this.findOne(id)
    if (note.serverId !== serverId) {
      throw new NotFoundException(`Nota "${id}" não encontrada`)
    }
